...
> [!NOTE]
> **"바이너리 직접 분석을 통한 고성능 모니터링"**
>
> 기존의 `hailortcli monitor` 명령어를 외부 프로세스로 실행하여 데이터를 수집하는 방식은 오버헤드가 크고 불안정할 수 있습니다.
> 반면, **Hailo Exporter**는 HailoRT의 내부 동작 방식을 역공학하여 **NPU 사용률 정보가 담긴 바이너리 파일을 직접 실시간 파싱**하는 독자적인 구조를 개발했습니다. 이를 통해 시스템 부하를 최소화하고, Prometheus 환경에 최적화된 안정적인 성능을 보장합니다.
